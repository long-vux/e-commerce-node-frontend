...
const Shopping = () => {
  ...
  // Extract unique categories, colors, and sizes from products
  const uniqueCategories = Array.from(new Set(products.flatMap(product => product.categories)));
  const uniqueColors = Array.from(new Set(products.flatMap(product => product.variants.map(variant => variant.color))));
  const uniqueSizes = Array.from(new Set(products.flatMap(product => product.variants.map(variant => variant.size))));

  // Define filters based on unique values
  const filters = [
    {
      label: 'Category',
      subLabels: uniqueCategories.length > 0 ? uniqueCategories : ['No categories available']
    },
    {
      label: 'Price Range',
    ...
    {
      label: 'Color',
      subLabels: uniqueColors.length > 0 ? uniqueColors : ['No colors available']
    },
    {
      label: 'Size',
  ...
  const filterProducts = () => {
    return products.filter(product => {
      const matchesCategory = selectedFilters[0].length === 0 || selectedFilters[0].some(selected => product.categories.includes(selected));
      const matchesColor = selectedFilters [2].length === 0 || selectedFilters[2].some(selected => product.variants.some(variant => variant.color === selected));
      const matchesSize = selectedFilters[3].length === 0 || selectedFilters[3].some(selected => product.variants.some(variant => variant.size === selected));
      const matchesPrice = selectedFilters[1].length === 0 || selectedFilters[1].some(selected => {
        if (selected === 'Under $20') return product.price < 20;
        if (selected === '$20 - $50') return product.price >= 20 && product.price <= 50;
        if (selected === '$50 - $100') return product.price > 50 && product.price <= 100;
