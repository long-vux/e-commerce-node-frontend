...
const Cart = () => {
  ...
  const fetchCartItems = async () => {
    try {
      ...
      const total = cartItems.reduce((total, item) => item.selected ? total + item.price : total, 0);
      setFinalTotal(total);
        ...
        <div className="flex flex-col gap-2">
                  ...
                  <td className="p-3 flex gap-1">
                    <div className="h-[140px] flex justify-center items-center">
                      <Checkbox
                        ...
                        color="default"
                        onChange={() => handleSelect(item._id, item.variant, item.selected)}
                      />
                    </div>
          ...
          <div className="border p-5 flex h-fit justify-end items-center gap-6">
            <p className="text-[20px]">
              <span> Total ({cartItems.filter(item => item.selected).length === 1 ? '1 item' : `${cartItems.filter(item => item.selected).length} items`}): </span>{' '}
              <span>
                <strong>{formatCurrency(finalTotal)}</strong>
              </span>
            </p>
            <button className="p-3 md:w-[300px] w-full rounded-sm hover:bg-white hover:text-black border-2 text-[10px] md:text-[15px] transition-all duration-200 font-semibold border-black bg-black text-white" onClick={() => navigate('/cart/checkout')}>
              CHECK OUT
            </button>
          </div>
        </div>
